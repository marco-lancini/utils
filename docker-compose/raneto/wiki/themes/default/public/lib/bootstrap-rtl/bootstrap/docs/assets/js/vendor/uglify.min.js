(function (exports, global) {
  ...
  var AST_String = DEFNODE(
    'String',
    'value',
    {$documentation: 'A string literal', $propdoc: {value: '[string] the contents of this string'}},
    AST_Constant
  );
  var AST_Number = DEFNODE(
    'Number',
    'value',
    {$documentation: 'A number literal', $propdoc: {value: '[number] the numeric value'}},
    AST_Constant
  );
  var AST_RegExp = DEFNODE(
    'RegExp',
    'value',
    {$documentation: 'A regexp literal', $propdoc: {value: '[RegExp] the actual regexp'}},
    AST_Constant
  );
  ...
  var RESERVED_WORDS =
    'abstract boolean byte char class double enum export extends final float goto implements import int interface long native package private protected public short static super synchronized this throws transient volatile yield' +
    ' ' +
    KEYWORDS_ATOM +
    ' ' +
    KEYWORDS;
  ...
  function parse($TEXT, options) {
    ...
    var object_ = embed_tokens(function () {
      ...
      while (!is('punc', '}')) {
        ...
        if (type == 'name' && !is('punc', ':')) {
          if (name == 'get') {
            a.push(
              new AST_ObjectGetter({start: start, key: as_atom_node(), value: function_(AST_Accessor), end: prev()})
            );
            continue;
          }
          if (name == 'set') {
            a.push(
              new AST_ObjectSetter({start: start, key: as_atom_node(), value: function_(AST_Accessor), end: prev()})
            );
            continue;
          }
        }
        expect(':');
        a.push(new AST_ObjectKeyVal({start: start, key: name, value: expression(false), end: prev()}));
      }
      next();
    ...
    var subscripts = function (expr, allow_calls) {
      ...
      if (allow_calls && is('punc', '(')) {
        next();
        return subscripts(new AST_Call({start: start, expression: expr, args: expr_list(')'), end: prev()}), true);
      }
      return expr;
    };
    ...
    var expr_op = function (left, min_prec, no_in) {
      ...
      if (prec != null && prec > min_prec) {
        ...
        return expr_op(
          new AST_Binary({start: left.start, left: left, operator: op, right: right, end: right.end}),
          min_prec,
          no_in
        );
      }
    ...
    var expression = function (commas, no_in) {
      ...
      if (commas && is('punc', ',')) {
        next();
        return new AST_Seq({start: start, car: expr, cdr: expression(true, no_in), end: peek()});
      }
      return expr;
    };
  ...
  SymbolDef.prototype = {
    unmangleable: function (options) {
      return (
        (this.global && !(options && options.toplevel)) ||
        this.undeclared ||
  ...
  AST_Scope.DEFMETHOD('has_directive', function (value) {
    return (
      (this.parent_scope && this.parent_scope.has_directive(value)) ||
  ...
  AST_Toplevel.DEFMETHOD('compute_char_frequency', function (options) {
    ...
    var tw = new TreeWalker(function (node) {
      ...
      else if (node instanceof AST_ObjectSetter) base54.consider('set' + node.key);
      else if (node instanceof AST_ObjectGetter) base54.consider('get' + node.key);
      ...
      else if (node instanceof AST_Symbol && node.unmangleable(options)) base54.consider(node.name);
      else if (node instanceof AST_Unary || node instanceof AST_Binary) base54.consider(node.operator);
  ...
  AST_Toplevel.DEFMETHOD('scope_warnings', function (options) {
    ...
    var tw = new TreeWalker(function (node) {
      ...
      if (options.assign_to_global) {
        var sym = null;
        if (node instanceof AST_Assign && node.left instanceof AST_SymbolRef) sym = node.left;
        else if (node instanceof AST_ForIn && node.init instanceof AST_SymbolRef) sym = node.init;
  ...
  (function () {
    ...
    PARENS(AST_New, function (output) {
      var p = output.parent();
      if (
        no_constructor_parens(this, output) &&
        (p instanceof AST_PropAccess || (p instanceof AST_Call && p.expression === this))
      )
        return true;
    });
    ...
    DEFPRINT(AST_UnaryPrefix, function (self, output) {
      ...
      if (
        /^[a-z]/i.test(op) ||
        (/[+-]$/.test(op) && self.expression instanceof AST_UnaryPrefix && /^[+-]/.test(self.expression.operator))
      ) {
        output.space();
      }
      self.expression.print(output);
    ...
    function regexp_safe_literal(code) {
      return (
        [92, 47, 46, 43, 42, 63, 40, 41, 91, 93, 123, 125, 36, 94, 58, 124, 33, 10, 13, 0, 65279, 8232, 8233].indexOf(
          code
        ) < 0
      );
    }
    ...
    function force_statement(stat, output) {
      if (output.option('bracketize')) {
        if (!stat || stat instanceof AST_EmptyStatement) output.print('{}');
        else if (stat instanceof AST_BlockStatement) stat.print(output);
        else
          output.with_block(function () {
            output.indent();
            stat.print(output);
  ...
  (function () {
    ...
    function tighten_body(statements, compressor) {
        ...
        function make_injector(func, name) {
          return make_node(AST_SimpleStatement, func, {
            body: make_node(AST_Assign, func, {
              operator: '=',
              left: make_node(AST_Dot, name, {expression: make_node(AST_SymbolRef, name, name), property: '$inject'}),
          ...
          switch (true) {
            ...
            case stat instanceof AST_If:
              if (stat.body instanceof AST_Return) {
                if (
                  ((in_lambda && ret.length == 0) || (ret[0] instanceof AST_Return && !ret[0].value)) &&
                  !stat.body.value &&
                  !stat.alternative
                ) {
                  CHANGED = true;
                ...
                if (
                  (ret.length == 0 || ret[0] instanceof AST_Return) &&
                  stat.body.value &&
                  !stat.alternative &&
                  in_lambda
                ) {
                ...
                if (
                  ret.length == 1 &&
                  in_lambda &&
                  ret[0] instanceof AST_SimpleStatement &&
                  (!stat.alternative || stat.alternative instanceof AST_SimpleStatement)
                ) {
                  CHANGED = true;
                  ret.push(
                    make_node(AST_Return, ret[0], {value: make_node(AST_Undefined, ret[0])}).transform(compressor)
                  );
              ...
              if (
                ab &&
                ((ab instanceof AST_Return && !ab.value && in_lambda) ||
        ...
        statements = statements.reduce(function (a, stat) {
          if (has_quit) {
            ...
            if (stat instanceof AST_LoopControl) {
              ...
              if (
                (stat instanceof AST_Break && lct instanceof AST_BlockStatement && loop_body(lct) === self) ||
      ...
      function sequencesize(statements, compressor) {
        ...
        function push_seq() {
          ...
          if (seq) ret.push(make_node(AST_SimpleStatement, seq, {body: seq}));
          seq = [];
        }
    ...
    (function (def) {
      ...
      def(AST_Binary, function () {
        return (
          member(this.operator, binary_bool) ||
          ((this.operator == '&&' || this.operator == '||') && this.left.is_boolean() && this.right.is_boolean())
        );
      });
          ...
          if (node instanceof AST_Definitions && !(tt.parent() instanceof AST_ForIn)) {
            ...
            if (side_effects.length > 0) {
              side_effects = make_node(AST_BlockStatement, node, {
                body: [make_node(AST_SimpleStatement, node, {body: AST_Seq.from_array(side_effects)})],
              });
            } else {
              side_effects = null;
            }
    ...
    function if_break_in_loop(self, compressor) {
      ...
      if (first instanceof AST_If) {
        if (first.body instanceof AST_Break && compressor.loopcontrol_target(first.body.label) === self) {
          ...
          first.alternative instanceof AST_Break &&
    ...
    OPT(AST_If, function (self, compressor) {
      ...
      if (
        ...
        return make_node(AST_SimpleStatement, self, {
          body: make_node(AST_Binary, self, {operator: '||', left: self.condition, right: self.alternative.body}),
        }).transform(compressor);
      }
      if (
        ...
        try {
          ...
          var tt = new TreeTransformer(function (node, descend, in_list) {
            if (node instanceof AST_Lambda || node instanceof AST_SimpleStatement) {
              ...
              return ruined
                ? node
                : make_node(AST_BlockStatement, node, {
        ...
        if (exp instanceof AST_SymbolRef && exp.undeclared()) {
          switch (exp.name) {
            ...
            case 'Number':
              ...
              if (self.args.length == 1)
                return make_node(AST_UnaryPrefix, self, {expression: self.args[0], operator: '+'}).transform(
                  compressor
                );
            case 'Boolean':
              ...
              if (self.args.length == 1)
                return make_node(AST_UnaryPrefix, self, {
                  expression: make_node(AST_UnaryPrefix, null, {expression: self.args[0], operator: '!'}),
                  operator: '!',
                }).transform(compressor);
              break;
            case 'Function':
    ...
    OPT(AST_Seq, function (self, compressor) {
      ...
      if (!self.car.has_side_effects(compressor)) {
        ...
        if (
          !(
            self.cdr instanceof AST_SymbolRef &&
            self.cdr.name == 'eval' &&
            self.cdr.undeclared() &&
      ...
      if (compressor.option('cascade')) {
        ...
        if (
          !self.car.has_side_effects(compressor) &&
      ...
      if (
        self.cdr instanceof AST_UnaryPrefix &&
    ...
    OPT(AST_Binary, function (self, compressor) {
      var reverse = compressor.has_directive('use asm')
        ? noop
        : function (op, force) {
      ...
      if (commutativeOperators(self.operator)) {
        ...
        if (/^[!=]==?$/.test(self.operator)) {
          if (self.left instanceof AST_SymbolRef && self.right instanceof AST_Conditional) {
            if (
              self.right.consequent instanceof AST_SymbolRef &&
            ...
            if (
              self.right.alternative instanceof AST_SymbolRef &&
          ...
          if (self.right instanceof AST_SymbolRef && self.left instanceof AST_Conditional) {
            if (
              self.left.consequent instanceof AST_SymbolRef &&
            ...
            if (
              self.left.alternative instanceof AST_SymbolRef &&
      ...
      if (compressor.option('booleans') && compressor.in_boolean_context())
        switch (self.operator) {
          ...
          case '+':
            ...
            var rr = self.right.evaluate(compressor);
            if (
              (ll.length > 1 && ll[0] instanceof AST_String && ll[1]) ||
      ...
      if (compressor.option('comparisons')) {
        if (!(compressor.parent() instanceof AST_Binary) || compressor.parent() instanceof AST_Assign) {
          var negated = make_node(AST_UnaryPrefix, self, {operator: '!', expression: self.negate(compressor)});
    ...
    OPT(AST_Undefined, function (self, compressor) {
      if (compressor.option('unsafe')) {
        ...
        if (undef) {
          var ref = make_node(AST_SymbolRef, self, {name: 'undefined', scope: scope, thedef: undef});
          ref.reference();
          return ref;
        }
      }
  ...
  function SourceMap(options) {
    ...
    function add(source, gen_line, gen_col, orig_line, orig_col, name) {
      if (orig_map) {
        var info = orig_map.originalPositionFor({line: orig_line, column: orig_col});
        if (info.source === null) {
          return;
        }
        source = info.source;
  ...
  (function () {
    var MOZ_TO_ME = {
      ...
      Property: function (M) {
        ...
        var args = {start: my_start_token(key), end: my_end_token(M.value), key: name, value: from_moz(M.value)};
        switch (M.kind) {
          case 'init':
      ...
      Identifier: function (M) {
        ...
        return new (p.type == 'LabeledStatement'
          ...
          ? AST_LabelRef
          : AST_SymbolRef)({start: my_start_token(M), end: my_end_token(M), name: M.name});
      },
    };
    ...
    def_to_moz(AST_ObjectProperty, function To_Moz_Property(M) {
      var key = is_identifier(M.key) ? {type: 'Identifier', name: M.key} : {type: 'Literal', value: M.key};
      var kind;
    ...
    function moz_sub_loc(token) {
      return token.line ? {line: token.line, column: token.col} : null;
    }
    function set_moz_loc(mynode, moznode) {
      ...
      if (start.line) {
        moznode.loc = {start: moz_sub_loc(start), end: moz_sub_loc(end)};
        if (start.file) {
