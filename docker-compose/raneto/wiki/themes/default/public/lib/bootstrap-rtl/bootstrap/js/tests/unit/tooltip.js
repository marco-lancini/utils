$(function () {
  ...
  test('should destroy tooltip', function () {
    ...
    ok(
      !$._data($tooltip[0], 'events').mouseover && !$._data($tooltip[0], 'events').mouseout,
  ...
  test('should correctly determine auto placement based on container rather than parent', function (assert) {
    ...
    $('#qunit-fixture').append(
      '<span id="trigger-parent"><a id="tt-trigger" title="If a_larger_text is written here, it won\'t fit using older broken version of BS">HOVER OVER ME</a></span>'
    );
    ...
    $trigger
      .on('shown.bs.tooltip', function () {
        ...
        ok(
          tipXrightEdge < triggerXleftEdge,
          'tooltip with auto left placement, when near the right edge of the viewport, gets left placement'
        );
        $trigger.bootstrapTooltip('hide');
      })
      .on('hidden.bs.tooltip', function () {
