...
(function (app, win) {
    ...
    var l = 'complete',
      ...
      r = function (e) {
        (e.type != s || i.readyState == l) &&
          ((e.type == 'load' ? n : i)[v](f + e.type, r, u), !h && (h = !0) && t.call(n, null));
      },
      o = function () {
        try {
          a.doScroll('left');
    ...
    if (mode == 'image') {
      ...
      if (!fallback) {
        el.style.backgroundImage =
          'url(' + draw({ctx: ctx, dimensions: dimensions, template: theme, ratio: ratio, holder: holder}) + ')';
  ...
  app.run = function (o) {
    ...
    for (var l = bgnodes.length, i = 0; i < l; i++) {
      ...
      if (flags) {
        ...
        var holder = parse_flags(
          bgsrc.substr(bgsrc.lastIndexOf(options.domain) + options.domain.length + 1).split('/'),
          options
        );
        if (holder) {
          render('background', bgnodes[i], holder, src);
    ...
    for (l = images.length, i = 0; i < l; i++) {
      ...
      if (src) {
        var holder = parse_flags(
          src.substr(src.lastIndexOf(options.domain) + options.domain.length + 1).split('/'),
          options
        );
        if (holder) {
          if (holder.fluid) {
