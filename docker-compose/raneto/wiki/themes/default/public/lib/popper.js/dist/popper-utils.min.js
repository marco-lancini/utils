 ...
 */ function a(
  ...
  const c = a.ownerDocument.defaultView,
...
function h(b) {
  ...
  let d = b.offsetParent || null;
  for (; d === c && b.nextElementSibling; ) d = (b = b.nextElementSibling).offsetParent;
...
function s(b) {
  ...
  const d = {left: c.left, top: c.top, width: c.right - c.left, height: c.bottom - c.top},
...
function t(b, d, e = !1) {
  ...
  let q = r({top: j.top - k.top - o, left: j.left - k.left - p, width: j.width, height: j.height});
  if (((q.marginTop = 0), (q.marginLeft = 0), !h && i)) {
    const a = parseFloat(n.marginTop, 10),
      b = parseFloat(n.marginLeft, 10);
    (q.top -= o - a), (q.bottom -= o - a), (q.left -= p - b), (q.right -= p - b), (q.marginTop = a), (q.marginLeft = b);
  }
...
function x(a, d, e, f, g = !1) {
  ...
  if ('viewport' === f) h = u(i, g);
    ...
    let e;
    'scrollParent' === f
      ? ((e = c(b(d))), 'BODY' === e.nodeName && (e = a.ownerDocument.documentElement))
      : 'window' === f
      ? (e = a.ownerDocument.documentElement)
      : (e = f);
    const j = t(e, i, g);
    if ('HTML' === e.nodeName && !v(i)) {
      const {height: b, width: c} = p(a.ownerDocument);
      (h.top += j.top - j.marginTop), (h.bottom = b + j.top), (h.left += j.left - j.marginLeft), (h.right = c + j.left);
    } else h = j;
  }
  e = e || 0;
  const j = 'number' == typeof e;
  return (
    (h.left += j ? e : e.left || 0),
    (h.top += j ? e : e.top || 0),
    (h.right -= j ? e : e.right || 0),
    (h.bottom -= j ? e : e.bottom || 0),
    h
  );
}
...
function z(a, b, c, d, e, f = 0) {
  ...
  const g = x(c, d, f, e),
    ...
    j = i.filter(({width: a, height: b}) => a >= c.clientWidth && b >= c.clientHeight),
...
function I(a) {
  let b;
  if ('HTML' === a.nodeName) {
    const {width: c, height: d} = p(a.ownerDocument);
    b = {width: c, height: d, left: 0, top: 0};
...
function Q(a, b, c) {
  ...
  if (!e) {
    const a = `\`${b}\``,
      d = `\`${c}\``;
    console.warn(`${d} modifier is required by ${a} modifier in order to work, be sure to include it before ${a}!`);
  }
  return e;
}
function R(a) {
...
function U(a, b, c) {
  ...
  return (
    d.forEach((a) => {
      ...
      a.enabled &&
        O(c) &&
        ((b.offsets.popper = r(b.offsets.popper)), (b.offsets.reference = r(b.offsets.reference)), (b = c(b, a)));
    }),
    b
  );
}
...
function W(a, b) {
  Object.keys(b).forEach((c) => {
    let d = '';
    -1 !== ['width', 'height', 'top', 'right', 'bottom', 'left'].indexOf(c) && R(b[c]) && (d = 'px'),
      (a.style[c] = b[c] + d);
  });
}
function X(a, b, d, e) {
  ...
  g.addEventListener(b, d, {passive: !0}), f || X(c(g.parentNode), b, d, e), e.push(g);
}
function Y(a, b, d, e) {
  (d.updateBound = e), S(a).addEventListener('resize', d.updateBound, {passive: !0});
  const f = c(a);
